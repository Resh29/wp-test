...
//Add bootstarp classes to menu items

const addBootstrapClasses = () => {
  const nav = document.querySelector('.bahama-nav');
  if (nav) {
    nav.querySelectorAll('.menu-item')?.forEach((item) => {
      item.classList.add('nav-item');
      item.querySelector('A').classList.add('nav-link');
      if (item.classList.contains('menu-item-has-children')) {
        item.firstElementChild.innerHTML += ' <i class="fas fa-chevron-down"></i>';
      }
      if (item.querySelector('.sub-menu')) {
        item.querySelector('.sub-menu').classList.add('dropdown-menu');
      }
    });
  } else {
    return;
